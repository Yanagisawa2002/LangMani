...
def test_phase2b2_contract_uses_independent_identity_and_exact_features() -> None:
    ...
    assert config.payload["expert_id"] == "PushToRegionExpert/CandidateH"
    assert config.payload["accepted_expert_commit"] == ("62db322a6954a362078badcb5f298501cb533bcf")
...
def test_atomic_shard_resume_reuses_verified_episode_and_rejects_tampering(
    ...
    resumed = push_collection._collect_shard_atomic(
        ...
        resume=True,
        expert_identity="PushToRegionExpert/CandidateH@62db322",
    )
