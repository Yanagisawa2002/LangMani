...
Ten later candidates were screened on disjoint diagnostic-only seeds; none justified consuming
...
workspace, so it also stopped at 5/64. Candidate P then reached 2/5 with one timeout, one planning
failure, and one cylinder workspace exit. The required `expert_success_rate >= 95%` condition
fails.
