"""Immutable evidence lifecycle for the M5A three-scene control screen."""
...
from langmani.language.three_scene_control import THREE_SCENE_SCREEN_SCHEMA


class ThreeSceneEvidenceError(RuntimeError):
    """Raised when paired-control evidence is unsafe or inconsistent."""
...
def _real_unlinked(value: str | Path, *, label: str) -> Path:
    ...
    for item in (path, *path.parents):
        if item.is_symlink() or item.is_junction():
            raise ThreeSceneEvidenceError(f"{label} traverses a link")
...
def _records(root: Path) -> list[dict[str, object]]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink() or path.is_junction():
            raise ThreeSceneEvidenceError("evidence contains a linked entry")
        if path.is_dir():
            continue
...
def _object(path: Path) -> dict[str, object]:
    try:
        ...
        raise ThreeSceneEvidenceError(f"cannot read {path.name}: {error}") from error
    if not isinstance(value, dict):
        raise ThreeSceneEvidenceError(f"{path.name} must contain one JSON object")
    return cast(dict[str, object], value)


def validate_three_scene_evidence(
    evidence_root: str | Path,
    *,
    expected_owner: Mapping[str, object] | None = None,
) -> dict[str, object]:
    root = _real_unlinked(evidence_root, label="three-scene evidence")
    if not root.is_dir():
        raise ThreeSceneEvidenceError("three-scene evidence root is missing")
    ...
    if expected_owner is not None and owner != dict(expected_owner):
        raise ThreeSceneEvidenceError("three-scene owner differs")
    records = _records(root)
    artifact_fingerprint = f"sha256:{sha256_hex({'owner': owner, 'artifacts': records})}"
    if manifest != {
        "schema_version": THREE_SCENE_SCREEN_SCHEMA,
        "artifact_fingerprint": artifact_fingerprint,
        "artifacts": records,
    }:
        raise ThreeSceneEvidenceError("three-scene manifest checksums differ")
    ...
    if (
        ...
        or complete.get("schema_version") != THREE_SCENE_SCREEN_SCHEMA
        ...
        raise ThreeSceneEvidenceError("three-scene completion marker differs")
    if (root / "complete.json").stat().st_mtime_ns < (root / "manifest.json").stat().st_mtime_ns:
        raise ThreeSceneEvidenceError("three-scene completion marker was not written last")
    return {
        "passed": True,
        "root": str(root),
        "owner": owner,
...
def write_three_scene_evidence(
    ...
    *,
    ...
    """Stage, fsync, validate, and atomically promote a completed screen."""

    root = _real_unlinked(output_root, label="three-scene output")
    run_fingerprint = owner.get("run_fingerprint")
    if not isinstance(run_fingerprint, str) or not run_fingerprint.startswith("sha256:"):
        raise ThreeSceneEvidenceError("owner requires a SHA-256 run fingerprint")
    token = run_fingerprint.removeprefix("sha256:")
    destination = root / token
    if destination.exists():
        return {
            **validate_three_scene_evidence(destination, expected_owner=owner),
    ...
    try:
        ...
        for relative, value in sorted(artifacts.items()):
            ...
            if pure.is_absolute() or ".." in pure.parts or pure.suffix not in {".json", ".md"}:
                raise ThreeSceneEvidenceError("three-scene artifact path is unsafe")
            if pure.name in {"owner.json", "manifest.json", "complete.json"}:
                raise ThreeSceneEvidenceError("three-scene artifact uses a reserved name")
        ...
        validate_three_scene_evidence(staging, expected_owner=owner)
        if staging.stat().st_dev != root.stat().st_dev:
            raise ThreeSceneEvidenceError("staging and destination use different filesystems")
