"""Verify M5A three-scene paired-control contracts or completed target evidence."""
...
from langmani.language.three_scene_control import THREE_SCENE_REJECTION_CASES  # noqa: E402
...
from scripts.run_language_control import (  # noqa: E402
    _validate_prior_one_scene_authority,
...
DEFAULT_REPORT = (
    PROJECT_ROOT / "outputs" / "diagnostics" / "m5a" / "three-scene-independent-verification.json"
)


class ThreeSceneVerificationCommandError(RuntimeError):
    """Raised when target-verification inputs are incomplete or unsafe."""
...
def _write(path: Path, value: dict[str, object]) -> None:
    ...
    if any(
        ...
        raise ThreeSceneVerificationCommandError("verification report traverses a link")
...
def _structural() -> dict[str, object]:
    categories = {case.category for case in THREE_SCENE_REJECTION_CASES}
    required = {
        "conflicting_objects",
        "conflicting_bins",
        "unsupported_action",
    ...
    passed = len(THREE_SCENE_REJECTION_CASES) == 6 and categories == required
    return {
        "schema_version": "langmani-m5a-three-scene-structural-verification-v0",
        "passed": passed,
...
def _target(args: argparse.Namespace) -> dict[str, object]:
    ...
    for name in required:
        if getattr(args, name) is None:
            raise ThreeSceneVerificationCommandError(f"--{name.replace('_', '-')} is required")
    git = inspect_git_state(PROJECT_ROOT)
    if not git.baseline_tracked or git.dirty:
        raise ThreeSceneVerificationCommandError(
    ...
    authority = _validate_prior_one_scene_authority(
    ...
    return verify_three_scene_control_evidence(
        args.evidence_root,
        inputs=inputs,
        registry=registry,
        source=source,
        prior_one_scene_authority=authority,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        traceback.print_exc()
        report = {
            "schema_version": "langmani-m5a-three-scene-verification-failure-v0",
            "passed": False,
